...
eps_sd <- 0.5
beta <- 0.6
x <- rnorm(n=m, mean=true_mean, sd=true_sd)
e <- rnorm(n=m, mean = eps_mean, sd=eps_sd)
...
```{r}
ynull <- rnorm(n=m, mean=0, sd=beta)
...
```{r}
ntests <- 7000
X <- matrix(rnorm(n=m*ntests, mean=true_mean, sd=true_sd), nrow=m, ncol=ntests)
epsilon <- matrix(rnorm(n=m*ntests, mean=eps_mean, sd=eps_sd), nrow=m, ncol=ntests)
dim(X) ; dim(epsilon)

Yalt <- X * beta + epsilon
Ynull <- matrix(rnorm(n=m, mean=0, sd=beta), nrow=m, ncol=ntests)
dim(Yalt) ; dim(Ynull)
```
...
wtrue <- sample(x=c(0,1), size=m, replace=TRUE, prob=c(1-0.2, 0.2))
...
```{r}
Yboth <- matrix(NA, nrow=m, ncol=ntests)
